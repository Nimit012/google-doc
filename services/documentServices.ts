import { 
  DocumentManager, 
  ProviderType,
  type GoogleDriveConfig,
  NotFoundError,
  ProviderError,
  ValidationError,
  PermissionError
} from 'document-management-service';

export class DocumentService {
  private static instance: DocumentService;
  private documentManager: DocumentManager | null = null;
  private initialized = false;

  private constructor() {}

  static getInstance(): DocumentService {
    if (!DocumentService.instance) {
      DocumentService.instance = new DocumentService();
    }
    return DocumentService.instance;
  }

  async initialize() {
    if (this.initialized && this.documentManager) return;

    const config: GoogleDriveConfig = {
      serviceAccountKey: {
        type: "service_account",
        project_id: process.env.GOOGLE_PROJECT_ID!,
        private_key_id: process.env.GOOGLE_PRIVATE_KEY_ID!,
        private_key: process.env.GOOGLE_PRIVATE_KEY!.replace(/\\n/g, '\n'),
        client_email: process.env.GOOGLE_CLIENT_EMAIL!,
        client_id: process.env.GOOGLE_CLIENT_ID!,
        auth_uri: "https://accounts.google.com/o/oauth2/auth",
        token_uri: "https://oauth2.googleapis.com/token",
        auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
        client_x509_cert_url: process.env.GOOGLE_CERT_URL!
      },
      adminEmail: process.env.ADMIN_EMAIL! // jasdeep.singh@comprotechnologies.com
    };

    this.documentManager = new DocumentManager({
      provider: ProviderType.GOOGLE_DRIVE,
      config
    });

    this.initialized = true;
    console.log('✅ DocumentService initialized');
  }

  getManager(): DocumentManager {
    if (!this.initialized || !this.documentManager) {
      throw new Error('DocumentService not initialized. Call initialize() first.');
    }
    return this.documentManager;
  }

  // Helper method to handle errors consistently
  handleError(error: any, context: string) {
    console.error(`❌ ${context}:`, error);
    
    
    
    throw error;
  }
}