...
export const useGoogleDrive = () => {
  // Configuration
  const CLIENT_ID =
    "252920647891-4948offll57nt51sri30io4k0v8am55f.apps.googleusercontent.com";
  ...
  const isAuthenticated = () => {
    const authenticated =
      accessToken.value &&
      tokenExpiry.value &&
      new Date().getTime() < tokenExpiry.value;
    console.log(
  ...
  const listPermissions = async (fileId) => {
    ...
    console.log(
      "🔑 Using access token:",
      accessToken.value ? "Token exists" : "Token is null"
    );
  ...
  const copyDocument = async (
    ...
    const copyResponse = await fetch(
      `https://www.googleapis.com/drive/v3/files/${originalFileId}/copy`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken.value}`,
  ...
  const createNamedVersion = async (fileId, versionName) => {
    try {
      console.log(
        `📝 Creating named version: "${versionName}" for file: ${fileId}`
      );

      // First, let's see what revisions already exist
      console.log("🔍 Checking existing revisions...");
      const initialRevisionsResponse = await fetch(
      ...
      const currentRevisionsResponse = await fetch(
        `https://www.googleapis.com/drive/v3/files/${fileId}/revisions`,
        {
          headers: {
            Authorization: `Bearer ${accessToken.value}`,
      ...
      if (currentRevisionsResponse.ok) {
        ...
        if (revisions.length > 0) {
          ...
          const nameCurrentResponse = await fetch(
            `https://www.googleapis.com/drive/v3/files/${fileId}/revisions/${latestRevision.id}`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${accessToken.value}`,
          ...
          if (nameCurrentResponse.ok) {
            ...
            console.log(
              "✅ Successfully named current revision:",
              namedRevision
            );
      ...
      console.log(
        "🔄 Trying to create new revision by making document change..."
      );
      ...
      if (commentResponse.ok) {
        ...
        const newRevisionsResponse = await fetch(
          `https://www.googleapis.com/drive/v3/files/${fileId}/revisions`,
          {
            headers: {
              Authorization: `Bearer ${accessToken.value}`,
        ...
        if (newRevisionsResponse.ok) {
          ...
          if (newRevisions.length > 0) {
            ...
            const nameNewResponse = await fetch(
              `https://www.googleapis.com/drive/v3/files/${fileId}/revisions/${latestRevision.id}`,
              {
                method: "PATCH",
                headers: {
            ...
            if (nameNewResponse.ok) {
              ...
              try {
                ...
                if (commentsListResponse.ok) {
                  ...
                  const versionComment = commentsData.comments?.find((c) =>
                    c.content?.includes("Version marker:")
                  );

                  if (versionComment) {
                    await fetch(
                      `https://www.googleapis.com/drive/v3/files/${fileId}/comments/${versionComment.id}`,
                      {
                        method: "DELETE",
                        headers: {
                          Authorization: `Bearer ${accessToken.value}`,
                ...
                console.warn(
                  "⚠️ Could not clean up version marker comment:",
                  cleanupError
                );
              }
            } else {
  ...
  const downloadAttemptVersion = async (
    fileId,
    revisionId,
    exportUrl,
    format
    ...
    try {
      ...
      const downloadFileName = `report.${format}`;
  ...
  const transferToTeacher = async (copiedDocId, teacherEmail, studentEmail) => {
    try {
      ...
      const studentPermission = permissions.find(
        (p) => p.emailAddress === studentEmail && p.role === "writer"
      );
      ...
      console.log(
        "✅ Document access transferred successfully with attempt stored"
      );
      return {
        success: true,
        attemptData,
  ...
  const finalizeDocument = async (copiedDocId, teacherEmail) => {
    try {
      ...
      const reviewVersion = await createNamedVersion(
        copiedDocId,
        reviewVersionName
      );
      ...
      const teacherPermission = permissions.find(
        (p) => p.emailAddress === teacherEmail && p.role === "writer"
      );
      ...
      console.log(
        "✅ Document finalized with review completion version - only admin has edit access, others have view access"
      );
      return {
        success: true,
        reviewVersion,
