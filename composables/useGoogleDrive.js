...
const API_KEY = 'AIzaSyCSrBjSZ5HuHNMT7JPfdnm1aRXHeL5pNZM'
  ...
  const fetchThumbnail = async (fileId) => {
    ...
    if (thumbnailLink) {
      console.log('Original thumbnail link:', thumbnailLink)
      thumbnailLink = thumbnailLink.replace(/=s220/, '=s1600')
      console.log('Modified thumbnail link:', thumbnailLink)
    }
    thumbnailCache[fileId] = thumbnailLink
    return thumbnailLink
  }

  const fetchCommentCount = async (fileId) => {
    try {
      const response = await fetch(
        `https://www.googleapis.com/drive/v3/files/${fileId}/comments?fields=comments(id)&includeDeleted=false`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken.value}`
          }
      ...
      if (!response.ok) {
        console.warn('Could not fetch comment count:', response.status)
        return 0
      }
      ...
      console.warn('Error fetching comment count:', error)
      return 0
    }
  }
